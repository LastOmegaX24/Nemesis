...
UGMC_OrganicMovementCmp::UGMC_OrganicMovementCmp(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
  ...
  EnableAccelerationForPathFollowing(true);
...
void UGMC_OrganicMovementCmp::MontageUpdate(float DeltaSeconds)
{
  ...
  const float PrevMontagePosition = MontageTracker.MontagePosition;
  ...
  const float MontageDelta = MontageTracker.MontagePosition - PrevMontagePosition;

  FLog(
    VeryVerbose,
    "Active montage was advanced, the updated parameters are: "
    "Montage = %s | MontagePosition = %f / %f | MontagePlayRate = %f | MontageDelta = %f",
  ...
  if (RootMotionParams.bHasRootMotion)
  {
    ...
    PreProcessRootMotion(MontageInstance, RootMotionParams, MontageDelta, DeltaSeconds);

    // Calculate the root motion velocity from the world space root motion translation.
    CalculateAnimRootMotionVelocity(ExtractionSettings, MontageDelta, DeltaSeconds);
    ...
    ApplyAnimRootMotionRotation(ExtractionSettings, MontageDelta, DeltaSeconds);
  }

  RootMotionParams.Clear();

  CallMontageEvents(MontageTracker, bFinishedBlendIn, bStartedBlendOut, MontageNotifyBeginEvents, MontageNotifyEndEvents, MontageDelta, DeltaSeconds);
...
)
{
  ...
  {
    OnMontageStarted(ActiveMontage, CurrentPosition, CurrentPlayRate, InOutMontageTracker.bInterruptedPreviousMontage, MontageDelta, DeltaSeconds);
  ...
  if (bFinishedBlendIn)
  {
    OnMontageBlendIn(ActiveMontage, CurrentPosition, CurrentPlayRate, MontageDelta, DeltaSeconds);
  ...
  if (bStartedBlendOut)
  {
    OnMontageBlendOut(ActiveMontage, CurrentPosition, CurrentPlayRate, MontageDelta, DeltaSeconds);
  ...
  for (const auto& Event : MontageNotifyBeginEvents)
  {
    ...
    OnMontageNotifyBegin(NotifyName, ActiveMontage, CurrentPosition, CurrentPlayRate, MontageDelta, DeltaSeconds);
  ...
  for (const auto& Event : MontageNotifyEndEvents)
  {
    ...
    const auto& NotifyName = GetNotifyName(Event);
    OnMontageNotifyEnd(NotifyName, ActiveMontage, CurrentPosition, CurrentPlayRate, MontageDelta, DeltaSeconds);
  ...
  {
    OnMontageCompleted(ActiveMontage, CurrentPosition, CurrentPlayRate, MontageDelta, DeltaSeconds);
...
void UGMC_OrganicMovementCmp::CalculateAnimRootMotionVelocity(
  const FGMC_RootMotionExtractionSettings& ExtractionSettings,
  float MontageDelta,
  float DeltaSeconds
)
{
  ...
  UpdateVelocity(CALL_NATIVE_EVENT_CONDITIONAL(bNoBlueprintEvents, this, PostProcessAnimRootMotionVelocity, RootMotionVelocity, MontageDelta, DeltaSeconds));

  FLog(VeryVerbose, "Applied final root motion velocity: %s", *Velocity.ToString())
}

void UGMC_OrganicMovementCmp::ApplyAnimRootMotionRotation(const FGMC_RootMotionExtractionSettings& ExtractionSettings, float MontageDelta, float DeltaSeconds)
{
...
{
  if (IsUsingFixedBrakingDistanceForPathFollowing())
  {
    return GetFixedBrakingDistanceForPathFollowing();
  }
