...
void UGMC_MovementUtilityCmp::RotateYawTowardsDirection(const FVector& Direction, float Rate, float DeltaTime)
{
  ...
  if (Rate <= 0.f)
  {
    UpdatedComponent->SetWorldRotation(FRotator(CurrentRotation.Pitch, TargetRotationYaw, CurrentRotation.Roll));
    return;
  }
  ...
  UpdatedComponent->AddWorldRotation(RotationDelta);
}
...
#else
  GetNavMovementProperties()->bUseAccelerationForPaths = bEnable;
#endif
}

bool UGMC_MovementUtilityCmp::IsUsingAccelerationForPathFollowing() const
{
  return UseAccelerationForPathFollowing();
}

bool UGMC_MovementUtilityCmp::IsUsingFixedBrakingDistanceForPathFollowing() const
{
#if UE_VERSION_OLDER_THAN(5, 5, 0)
  return bUseFixedBrakingDistanceForPaths;
#else
  return GetNavMovementProperties().bUseFixedBrakingDistanceForPaths;
#endif
}

float UGMC_MovementUtilityCmp::GetFixedBrakingDistanceForPathFollowing() const
{
#if UE_VERSION_OLDER_THAN(5, 5, 0)
  return FixedPathBrakingDistance;
#else
  return GetNavMovementProperties().FixedPathBrakingDistance;
#endif
}
