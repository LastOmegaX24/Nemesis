...
void UGMC_ReplicationCmp::UpdateRemotelyControlledServerPawn()
{
  ...
  {
    ArrayRemoveAt(SV_RemoteMoveExecutionAux.PendingMoves, 0, NumMovesToExecute, false);
  }
  else
  {
...
bool UGMC_ReplicationCmp::SV_AuditClientMoves(TArray<FGMC_Move>& RemoteMoves)
{
  ...
  if (MoveHistory.Num() > 0)
  {
    for (int32 Index = 0; Index < RemoteMoves.Num(); ++Index)
    {
      ...
      ArrayRemoveAt(RemoteMoves, Index, 1, false);
      --Index;
    }
  }
...
bool UGMC_ReplicationCmp::SV_CheckForcedNetUpdate(const FGMC_Move& Move, bool bValidMove)
{
  ...
  {
    if (!bValidMove && GetOwnerNetUpdateFrequency() <= 0.f)
    {
...
bool UGMC_ReplicationCmp::CL_MaintainNonPredictedMoves(const FGMC_Move& NewMove, bool& bOutStartedNewMove)
{
  ...
  if ((bOutStartedNewMove = CL_ShouldEnqueueMove(NewMove, PreviousMove, bCombineMove)) == true)
  {
    ...
    {
      ArrayRemoveAt(NonPredictedMoves, 0, 1, false);
    }
...
)
{
  ...
  if (!bIsUsingPhysics)
  {
    const auto SetParamsSimulationBased = [this](auto& Params, float DeltaTime, bool bForceExtrap)
    {
      ...
      gmc_ck(IsValid(PawnOwner))
      const float UpdateFrequency = IsRemotelyControlledServerPawn() ? GetClientSendRate() : GetOwnerNetUpdateFrequency();
...
) const
{
  ...
  else
  {
...

    {
      ...
      const float AvgNetUpdateDelay = 1.f / FMath::Max(OtherReplicationComponent->GetOwnerNetUpdateFrequency(), UE_KINDA_SMALL_NUMBER) / 2.f;
...
float UGMC_ReplicationCmp::GetOwnerNetUpdateFrequency() const
{
  if (!IsValid(PawnOwner))
  {
    return 0.f;
  ...
  return PawnOwner->NetUpdateFrequency;

#else

  return PawnOwner->GetNetUpdateFrequency();
...
{
  gmc_ck(IsValid(Base) ? Base->GetOwner() != GetOwner() : true)

  if (IsValid(Base) && Base->GetOwner() != GetOwner())
  {
    ActorBase = Base;
    return;
  }
...
{
  gmc_ck(IsValid(ActorBase) ? ActorBase->GetOwner() != GetOwner() : true)
...
void UGMC_ReplicationCmp::FPredictedClientNetSerializationAux::PreTick(const UGMC_ReplicationCmp* const Outer, float DeltaTime)
{
  ...
  UpdateFrequency = FMath::Min(Outer->PredictedClientNetUpdateFrequency, Outer->GetOwnerNetUpdateFrequency());
}
