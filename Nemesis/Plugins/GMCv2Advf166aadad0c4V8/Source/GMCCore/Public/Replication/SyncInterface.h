...
#define ADD_STRUCT_REFERENCED_OBJECTS(Name)\
  Name.AddStructReferencedObjects(Collector);
...
struct GMCCORE_API FGMC_PawnState
{
  ...
  void AddStructReferencedObjects(FReferenceCollector& Collector) { FOR_EACH(ADD_STRUCT_REFERENCED_OBJECTS, ALL_SYNC_TYPES) }
};

template<>
struct TStructOpsTypeTraits<FGMC_PawnState> : TStructOpsTypeTraitsBase2<FGMC_PawnState>
{
  enum { WithAddStructReferencedObjects = true };
};

#undef ADD_SYNC_TYPE
#undef ADD_STRUCT_REFERENCED_OBJECTS
