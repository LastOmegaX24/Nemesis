...
template<typename T>
inline void ArrayRemoveAt(TArray<T>& InOutArray, int32 Index, int32 Count, bool bAllowShrinking)
{
#if UE_VERSION_OLDER_THAN(5, 5, 0)
  InOutArray.RemoveAt(Index, Count, bAllowShrinking);
#else
  InOutArray.RemoveAt(Index, Count, static_cast<EAllowShrinking>(bAllowShrinking));
#endif
}
